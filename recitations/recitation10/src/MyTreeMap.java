	  ...
	  public V remove(K key) throws IllegalArgumentException {
	    ...
	    if (isExternal(p)) {                    // key not found
	      ...
	      if (isInternal(left(p)) && isInternal(right(p))) { // both children are internal
	        Position<Entry<K,V>> x1 = treeMax(left(p));
	        Position<Entry<K,V>> x2 = treeMin(right(p));
	        Position<Entry<K,V>> replacement = null;
	        
	        int old2 = (int) old;
	        int v1 = Math.abs(old2 - (int) x1.getElement().getValue());
	        int v2 = Math.abs(old2 - (int) x2.getElement().getValue());
