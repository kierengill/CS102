...
public class MyTreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  public int getHeight(Position<Entry<K,V>> p) {
	    ...
	    int lefth = getHeight(left(p));
	    int righth = getHeight(right(p));
  ...
  public int searchDepth(K key) {
	  Position<Entry<K,V>> p = root();
	  return searchDepth(p, key);
	  }
  
  public int searchDepth(Position<Entry<K,V>> p, K key) {
	    int comp = compare(key, p.getElement());
	    if (comp == 0)
	      return 0;                          // key found; return its position
	    else if (comp < 0)
	      return searchDepth(left(p), key) + 1;   // search left subtree
	    else
	      return searchDepth(right(p), key) + 1;  // search right subtree
	  }
}
